...
if newpass==cpass:
 n=cur.execute(q)
 if n==1:
  print "<script>alert('Password Updated Succesfully');window.location.href='../login.py';</script>"
 else:
  print "<script>alert('Password not updated');window.location.href='changepwd.py';</script>"
else:
 print "<script>alert('Password not updated');window.location.href='changepwd.py';</script>"